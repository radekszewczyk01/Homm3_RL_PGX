...
Mierzy:
  ...
  4. FLOP-y i bajty (cost_analysis) -> intensywnosc arytmetyczna
  5. przepustowosc decyzji MCTS wzgledem wsadu i wzgledem num_simulations
  6. pobor mocy -> dzule na decyzje

UZYCIE
    # autotest zaleznosci - zrob to najpierw
    python3 bench.py --selftest

    # pomiary
    CUDA_VISIBLE_DEVICES=0     python3 bench.py --tag 1gpu --engine v3 --power
    CUDA_VISIBLE_DEVICES=0,1   python3 bench.py --tag 2gpu --engine v3 --power
    CUDA_VISIBLE_DEVICES=0,1,2 python3 bench.py --tag 3gpu --engine v3 --power
    CUDA_VISIBLE_DEVICES=0     python3 bench.py --tag 1gpu --engine v2 --no-mcts

    # doprecyzowanie progu wydajnosci (gesta siatka, wiecej powtorzen)
    CUDA_VISIBLE_DEVICES=0 python3 bench.py --tag prog --engine v3 --no-mcts \\
        --batches 1024 1536 2048 2560 3072 3584 4096 6144 8192 --repeats 7
...
def build_mcts(env, mod, batch, n_sims):
    ...
    def recurrent_fn(params, rng, action, st):
        ...
        discount = jnp.where(st.terminated, 0.0, -1.0)
...
def selftest():
    ...
    silnika. mctx 0.0.5 nie byl aktualizowany od 2023 r. i nie wspolpracuje
    z jax >= 0.5.
    """
    print("=" * 74)
    ...
    try:
        ...
        print("     z przypietymi wersjami: jax 0.4.38, mctx 0.0.5, "
              "chex 0.1.88.")
        ok = False
...
def run_suite(args):
    ...
    save()   # plik istnieje od poczatku - Ctrl+C nie niszczy pracy

    # ---------------------------------------------------------------
    print(f"\n[1/3] Kroki symulacji ({args.steps} krokow na pomiar)")
    print(f"{'wsad':>7} {'kroki/s':>14} {'czas':>9} {'lower':>7} "
          f"{'compile':>8} {'1. wyk.':>9} {'pamiec':>9} {'I=F/B':>7}")
    for b in batches:
        try:
            r = bench_steps(env, b, args.steps, sharding, args.repeats,
                            args.analysis)
        except Exception as exc:
            print(f"{b:>7}   PRZERWANO: {type(exc).__name__}: {str(exc)[:70]}")
            if args.debug:
                traceback.print_exc()
        ...
        print(f"{b:>7} {r['steps_per_s']:>14,.0f} {r['wall_s_best']:>9.3f} "
              ...
              f"{r['first_run_s']:>9.3f} {mem:>9} {ai_s:>7}")
    ...
    if not args.no_mcts:
        ...
        print(f"{'wsad':>7} {'decyzje/s':>12} {'kroki/s':>14} "
              f"{'compile':>8} {'pamiec':>9} {'moc [W]':>9} {'J/dec':>9}")
        for b in [x for x in batches if x <= args.max_mcts_batch]:
            try:
                ...
                print(f"{b:>7}   PRZERWANO: {type(exc).__name__}: "
                      f"{str(exc)[:70]}")
                traceback.print_exc()
                record_error(f"mcts[batch={b}]", exc)
                print("\n  Uruchom  python3 bench.py --selftest  zeby ustalic,")
                print("  czy przyczyna lezy po stronie wersji bibliotek.")
                break
        ...
        if results["mcts"] and not args.no_sims_sweep:
            ...
            for ns in args.sims_list:
                try:
                    ...
                    if args.debug:
                        traceback.print_exc()
                    record_error(f"sims_sweep[n={ns}]", exc)
                    break
    ...
    if results["steps"]:
        ...
        if len(pts) >= 3:
            ...
            print(f"Pamiec na stan: {slope:>14,.0f} B "
                  f"(narzut staly {inter/2**20:,.0f} MiB)")
        ai = [r.get("arithmetic_intensity") for r in results["steps"]
              if r.get("arithmetic_intensity")]
        if ai:
            m = float(np.mean(ai))
            lim = ("ograniczenie przepustowoscia pamieci" if m < 10
                   else "ograniczenie moca obliczeniowa")
            print(f"Intensywnosc:   {m:>14.2f} FLOP/bajt  -> {lim}")
    if results["mcts"]:
        ...
        print(f"                {d*3600:>14,.0f} /h, {d*3600*24:,.0f} /dobe")
        if pm.get("joules_per_decision"):
            print(f"Energia:        {pm['joules_per_decision']:>14.4f} "
                  f"J/decyzje")
